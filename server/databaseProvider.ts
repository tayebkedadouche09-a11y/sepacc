...
export function getCustomerDbProviderStatus(): {
  ...
  if (p === "planetscale") {
    const ok = Boolean(ENV.planetscaleServiceToken && ENV.planetscaleOrg);
    return {
      provider: "planetscale",
      configured: ok,
      detail: ok ? "PlanetScale token present" : "Set PLANETSCALE_SERVICE_TOKEN + PLANETSCALE_ORG",
    };
  }
...
export async function createCustomerDatabase(opts: {
  ...
  try {
    ...
    return {
      status: "ERROR",
      provider: status.provider,
      error: e instanceof Error ? e.message : "Customer DB create failed",
    };
  }
}
