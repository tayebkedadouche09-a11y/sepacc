...
export async function createVercelProject(repo: { owner: string; repo: string; branch: string }, slug: string) {
  ...
  const project = await vercelRequest<{ id?: string; name?: string; targets?: { production?: { url?: string } }; alias?: string[] }>(`/v10/projects${teamQuery}`, {
    method: "POST",
    body: JSON.stringify({ name: projectName, gitRepository: { type: "github", repo: `${repo.owner}/${repo.repo}`, productionBranch: repo.branch } }),
  });
